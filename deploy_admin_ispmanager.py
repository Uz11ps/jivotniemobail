"""
Скрипт деплоя админ-панели на сервер с ISPmanager.
"""
import os
import sys
import tarfile
...
def deploy():
    ...
    # Настройка nginx через ISPmanager (создаём конфиг для ручной настройки)
    nginx_config = f"""# Конфигурация для ISPmanager
# Добавьте это в настройки сайта через веб-интерфейс ISPmanager
# или создайте файл в /etc/nginx/conf.d/deti-admin.conf
...
"""
    ...
    # Сохраняем конфиг для ручной настройки через ISPmanager
    run(f"cp {tmp_path} {REMOTE_DIR}/nginx-config.conf")
    print(f"Конфигурация nginx сохранена в {REMOTE_DIR}/nginx-config.conf")
    print("Настройте проксирование через веб-интерфейс ISPmanager")
    ...
    print(f"Файлы находятся в: {REMOTE_DIR}")
    print("\nСледующие шаги:")
    print("1. Настройте .env.local с Firebase credentials")
    print("2. Настройте проксирование через ISPmanager веб-интерфейс")
    print("3. Используйте конфиг из nginx-config.conf для настройки")
