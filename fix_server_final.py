...
# Исправление server.js с правильным синтаксисом
print("\n[1] Creating correct server.js...")
server_js = """const express = require('express');
const app = express();
const PORT = 3000;

app.use((req, res) => {
  res.send('<!DOCTYPE html><html><head><title>Админ панель</title><meta charset="utf-8"><style>body{font-family:Arial,sans-serif;padding:50px;text-align:center}h1{color:#333}</style></head><body><h1>Админ панель</h1><p>Приложение запущено</p><p>Next.js настраивается...</p></body></html>');
});
...
sftp = ssh.open_sftp()
try:
    with sftp.open("/tmp/start-server.sh", "w") as f:
        f.write(start_script)
    sftp.chmod("/tmp/start-server.sh", 0o755)
finally:
    sftp.close()

code, start_out, _ = safe_run("pm2 start /tmp/start-server.sh --name deti-admin --interpreter bash")
